...
if IsServer then
    ...
    local function Clean(inst, level)
        ...
        for k, v in pairs(this_max_prefabs) do
            if countList[k] ~= nil and countList[k].count > v.max then
                print(string.format("[清酒]清理   %s(%s)   %d   %d   %d", countList[k].name, k, countList[k].count, countList[k].count - countList[k].currentcount, countList[k].currentcount))
            end
        end
    end
    ...
    _G.hx_clean_custom = function(custom_prefabs)
        ...
        print("[清酒]设置自定义清理物品", custom_prefabs)
    ...
    _G.hx_clean_list = function()
        ...
        for k, v in pairs(max_prefabs) do
            if countList[k] ~= nil then
                print("[清酒]", countList[k].name .. "(" .. k .. ")", countList[k].count)
            end
        end
    end
end