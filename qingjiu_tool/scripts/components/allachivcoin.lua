...
function allachivcoin:jumpcoin(inst)
    if self.jump ~= true and self.coinamount >= allachiv_coinuse["jump"] then
        self.jump = true
        self:coinDoDelta(-allachiv_coinuse["jump"])
        self:ongetcoin(inst)
    end
end
...
--快速采集
function allachivcoin:fastpickercoin(inst)
    if self.fastpicker ~= true and self.coinamount >= allachiv_coinuse["fastpicker"] then
        self.fastpicker = true
        self:coinDoDelta(-allachiv_coinuse["fastpicker"])
        self:fastpickerfn(inst)
        self:ongetcoin(inst)
    end
end
...
function allachivcoin:critcoin(inst)
    if self.coinamount >= allachiv_coinuse["crit"] and self.crit <= 19 then
