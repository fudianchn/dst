...
function MakeBoneWallType(data)
    ...
    local function ondeploywall(inst, pt, deployer)
        ...
        local bonewall = SpawnPrefab("wall_"..data.name) 
        if bonewall ~= nil then 
            pt = Vector3(math.floor(pt.x) + .5, 0, math.floor(pt.z) + .5)


            bonewall.Physics:SetCollides(false)
            bonewall.Physics:Teleport(pt:Get())
            bonewall.Physics:SetCollides(true)
...
local bonewallprefabs = {}

local bonewalldata =
{
	{name = "bone", tags={"bone", "stone"}, loot = "boneshard", maxloots = 2, maxhealth=350, buildsound="dontstarve/common/place_structure_stone", destroysound="dontstarve/common/destroy_stone"}
}

for k,v in pairs(bonewalldata) do
    local bonewall, item, placer = MakeBoneWallType(v)
    table.insert(bonewallprefabs, bonewall)
    table.insert(bonewallprefabs, item)
    table.insert(bonewallprefabs, placer)
end

return unpack(bonewallprefabs)
