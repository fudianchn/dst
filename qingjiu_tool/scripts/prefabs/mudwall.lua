...
function MakeMudWallType(data)
    ...
    local function ondeploywall(inst, pt, deployer)
        ...
        local mudwall = SpawnPrefab("wall_"..data.name) 
        if mudwall ~= nil then 
            pt = Vector3(math.floor(pt.x) + .5, 0, math.floor(pt.z) + .5)


            mudwall.Physics:SetCollides(false)
            mudwall.Physics:Teleport(pt:Get())
            mudwall.Physics:SetCollides(true)
...
local mudwallprefabs = {}

local mudwalldata =
{
	{name = "mud", tags={"mud", "hay"}, loot = "poop", maxloots = 2, maxhealth=150, buildsound="dontstarve/common/place_structure_wood", destroysound="dontstarve/common/destroy_wood"}
}

for k,v in pairs(mudwalldata) do
    local mudwall, item, placer = MakeMudWallType(v)
    table.insert(mudwallprefabs, mudwall)
    table.insert(mudwallprefabs, item)
    table.insert(mudwallprefabs, placer)
end

return unpack(mudwallprefabs)
