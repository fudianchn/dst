...
function MakeHedgeWallType(data)
    ...
    local function ondeploywall(inst, pt, deployer)
        ...
        local hedgewall = SpawnPrefab("wall_hedge") 
        if hedgewall ~= nil then 
            pt = Vector3(math.floor(pt.x) + .5, 0, math.floor(pt.z) + .5)


            hedgewall.Physics:SetCollides(false)
            hedgewall.Physics:Teleport(pt:Get())
            hedgewall.Physics:SetCollides(true)
...
local hedgewallprefabs = {}

local hedgewalldata =
{
	{name = "hedge", tags={"hedge", "pinecone"}, loot = "pinecone", maxloots = 2, maxhealth=450, flammable = true, buildsound="dontstarve/wilson/pickup_plants", destroysound="dontstarve/common/destroy_wood"}
}

for k,v in pairs(hedgewalldata) do
    local hedgewall, item, placer = MakeHedgeWallType(v)
    table.insert(hedgewallprefabs, hedgewall)
    table.insert(hedgewallprefabs, item)
    table.insert(hedgewallprefabs, placer)
end

return unpack(hedgewallprefabs)
