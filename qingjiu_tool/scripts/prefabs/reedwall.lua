...
function MakeReedWallType(data)
    ...
    local function ondeploywall(inst, pt, deployer)
        ...
        local reedwall = SpawnPrefab("wall_"..data.name) 
        if reedwall ~= nil then 
            pt = Vector3(math.floor(pt.x) + .5, 0, math.floor(pt.z) + .5)


            reedwall.Physics:SetCollides(false)
            reedwall.Physics:Teleport(pt:Get())
            reedwall.Physics:SetCollides(true)
...
local reedwallprefabs = {}

local reedwalldata =
{
	{name = "reed", tags={"reed", "hay"}, loot = "cutreeds", maxloots = 2, maxhealth=250, flammable = true, buildsound="dontstarve/common/place_structure_wood", destroysound="dontstarve/common/destroy_wood"}
}

for k,v in pairs(reedwalldata) do
    local reedwall, item, placer = MakeReedWallType(v)
    table.insert(reedwallprefabs, reedwall)
    table.insert(reedwallprefabs, item)
    table.insert(reedwallprefabs, placer)
end

return unpack(reedwallprefabs)
